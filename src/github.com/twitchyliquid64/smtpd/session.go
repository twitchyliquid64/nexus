...
func (s *Server) startSession(id int, conn net.Conn) {
	log.Printf("SMTP Connection from %v, starting session <%v>", conn.RemoteAddr(), id)
	defer func() {
		if err := conn.Close(); err != nil {
			log.Printf("Error closing connection for <%v>: %v", id, err)
		}
		s.waitgroup.Done()
	}()
		...
		if err == nil {
			if cmd, arg, ok := ss.parseCmd(line); ok {
				...
				case "SEND", "SOML", "SAML", "EXPN", "HELP", "TURN":
					...
					ss.logWarn("Command %v not implemented by us", cmd)
					continue
				case "VRFY":
...
func (ss *Session) dataHandler() {
	...
	for e := ss.recipients.Front(); e != nil; e = e.Next() {
		...
		err = ss.server.dataStore.AcceptRecipient(local)
		if err != nil {
...
func (ss *Session) deliverMessage(r recipientDetails, msgBuf [][]byte) (ok bool) {
	...
	if _, err := msg.Write([]byte(recd)); err != nil {
	...
	for _, line := range msgBuf {
		if _, err := msg.Write(line); err != nil {
			ss.logError("Failed to append to message: %v", err)
			return false
		}
	}

	if err := ss.server.dataStore.Commit(msg.Bytes(), &MsgMetadata{
		TLS:        false,
		From:       ss.from,
		Remote:     ss.remoteHost,
		Domain:     ss.remoteDomain,
		Recipients: ss.recipients,
	}); err != nil {
		ss.logError("message transaction failed: %v", err)
		return false
	}
	return true
}
...
func (ss *Session) logTrace(msg string, args ...interface{}) {
	log.Printf("SMTP[%v]<%v> %v", ss.remoteHost, ss.id, fmt.Sprintf(msg, args...))
}
